  ...
  return (
    <button
      ...
      className={clsx(
        ...
        selected ? "border-amber-400 ring-2 ring-amber-400/40" : "border-zinc-800 hover:border-zinc-600",
      )}
    >
      <div className="aspect-video relative" style={{ background: presetGradient(entry.name) }}>
        ...
        >
          <Heart size={14} className={favorite ? "fill-amber-400 text-amber-400" : ""} />
        </button>
        {selected && (
      ...
      <div className="p-2.5 bg-zinc-900">
        <p className="text-xs font-medium text-zinc-200 line-clamp-2 leading-snug">{entry.name}</p>
        <div className="flex items-center gap-1.5 mt-1">
          <p className="text-[10px] text-zinc-500 truncate">{entry.author}</p>
          <span className="text-[9px] px-1 py-0.5 rounded bg-zinc-800 text-zinc-400 uppercase tracking-wider">{entry.category}</span>
        </div>
      </div>
    </button>
  );
...
export default function PresetsPage() {
  ...
  const [selectedIndex, setSelectedIndex] = useState(() => loadLastPresetIndex());
      ...
      <div className="shrink-0 border-b border-zinc-800 bg-zinc-950">
        <div className="px-4 py-3 flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-semibold text-zinc-100">Preset browser</h1>
            {!loading && (
              <span className="text-xs text-zinc-500 font-mono">{filtered.length} / {presets.length}</span>
            )}
          </div>
        ...
        <div className="h-52 sm:h-64 lg:h-72 border-t border-zinc-800">
          <PresetPreview preset={selected} className="h-full w-full" transitionSec={slideshow ? 2.5 : 1.5} />
        </div>
        ...
        {selected && (
          <div className="px-4 py-2 border-t border-zinc-800 bg-zinc-900/80">
            ...
            <p className="text-xs text-zinc-500">{selected.author} <span className="text-[10px] text-zinc-600 ml-1.5 uppercase tracking-wider">{selected.category}</span></p>
          </div>
        )}
      </div>

      <div className="flex-1 overflow-y-auto p-4">
        {loading && <p className="text-sm text-zinc-500">Loading preset library…</p>}
        {error && <p className="text-sm text-red-400">{error}</p>}

        {!loading && filtered.length === 0 && (
          <p className="text-sm text-zinc-500 text-center py-12">No presets match your filters</p>
        )}
