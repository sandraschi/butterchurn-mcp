  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="p-6 max-w-4xl pb-8 overflow-y-auto h-full">
      <div className="mb-6">
        ...
        <p className="text-sm text-zinc-500 mt-0.5">MCP tools registered on butterchurn-mcp</p>
      </div>
      ...
      <div className="space-y-4">
        {tools.map((t) => (
          <div key={t.name} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
            <div className="flex items-start gap-3">
              <Terminal size={20} className="text-amber-400 shrink-0 mt-0.5" />
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-mono text-sm text-amber-300">{t.name}</span>
                  <span className="text-xs px-2 py-0.5 rounded bg-zinc-700/50 text-zinc-400">{t.kind}</span>
                </div>
