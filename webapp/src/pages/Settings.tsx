...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const detectedNames = detected.filter((p) => p.status === "detected").map((p) => p.name);
      const preferred = saved.provider && detectedNames.includes(saved.provider) ? saved.provider : detectedNames[0] || "";
      ...
      if (preferred) {
        ...
        if (p) {
          ...
          if (!mountedRef.current) return;
          setModels(ms);
          const preferredModel = saved.model && ms.includes(saved.model) ? saved.model : ms[0] || "";
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="p-6 max-w-2xl pb-8 overflow-y-auto h-full space-y-8">
      <div>
        <h1 className="text-xl font-semibold text-zinc-100">Settings</h1>
        <p className="text-sm text-zinc-500 mt-0.5">Backend and LLM configuration</p>
      </div>

      <section className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-4">
        <div className="flex items-center gap-2">
          <Cpu size={16} className="text-amber-400" />
          <h2 className="text-sm font-semibold text-zinc-200">Local Intelligence</h2>
        </div>
        ...
        {probing ? (
          ...
          <div className="bg-amber-900/20 border border-amber-800/30 rounded-lg px-4 py-3 text-sm text-amber-300">
            ...
            <a href="https://ollama.com" className="underline hover:text-amber-200" target="_blank" rel="noreferrer">Ollama</a>{" "}
            or <a href="https://lmstudio.ai" className="underline hover:text-amber-200" target="_blank" rel="noreferrer">LM Studio</a>{" "}
          ...
          <div className="space-y-3">
              ...
              {providers.map((p) => (
                <div
                  ...
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs ${
                    p.status === "detected" ? "bg-emerald-900/30 text-emerald-300" : "bg-zinc-800 text-zinc-500"
                  }`}
                >
                  {p.status === "probing" ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                >
                  {providers.filter((p) => p.status === "detected").length === 0 && (
                    <option value="">No provider detected</option>
                  )}
                  {providers.filter((p) => p.status === "detected").map((p) => (
