...
function Inner({ backendOk }: { backendOk: boolean | null }) {
  ...
  const { data: dash } = useQuery({
    queryKey: ["dashboard"],
    queryFn: () => fetch("/api/dashboard").then((r) => r.json()) as Promise<DashboardData>,
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="p-6 max-w-4xl pb-8 overflow-y-auto h-full">
      <div className="mb-6">
        ...
        <p className="text-sm text-zinc-500 mt-0.5">MilkDrop WebGL visualizer — BPM sync for mixx-dj-mcp</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard label="BPM" value={dash ? String(dash.bpm) : "—"} icon={Heart} color="text-amber-400" />
        <StatCard
          label="Uptime"
        ...
        <StatCard
          label="Tools"
          value={caps ? String(caps.tool_surface.total) : capsLoading ? "…" : "—"}
          icon={Server}
          color="text-violet-400"
        />
        <StatCard label="Shader scenes" value="6" icon={Sparkles} color="text-cyan-400" />
        <StatCard label="MilkDrop" value="500+" icon={Music2} color="text-amber-400" />
      </div>
...
export default function Dashboard({ backendOk }: { backendOk: boolean | null }) {
  return (
